...
class Chaincode extends Contract {
	...
	async CreateAsset(ctx, assetID, holder, issuer, hash) {
		...
		let indexName = 'issuer';
		let issuerIndexKey = await ctx.stub.createCompositeKey(indexName, [asset.issuedBy, asset.assetID]);
		...
		await ctx.stub.putState(issuerIndexKey, Buffer.from('\u0000'));
	}
	...
	async QueryAssetsByholder(ctx, holder) {
		let queryString = {};
		queryString.selector = {};
		queryString.selector.docType = 'asset';
		queryString.selector.owner = holder;
	...
	// Only available on state databases that support rich query (e.g. CouchDB)
	// Example: Parameterized rich query
	async QueryAssetsByissuer(ctx, issuedBy) {
		let queryString = {};
		queryString.selector = {};
		queryString.selector.docType = 'asset';
		queryString.selector.issuedBy = issuedBy;
		return await this.GetQueryResultForQueryString(ctx, JSON.stringify(queryString)); //shim.success(queryResults);
	}
